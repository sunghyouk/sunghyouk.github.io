...
updated : 2022-01-15 18:32:19 +0900
tag     : 
toc     : true
public  : true
...
리스트는 어떨까?  
리스트는 **mutable** 객체이지만 리스트 내부 원소인 각 숫자는 **immutable** 객체로 서로 다른 메모리 주소를 지닌다.  
...
얕은 복사는 **mutable** 리스트 객체 내부에 **immutable** 객체인 숫자형 변수가 존재할 때 사용  
그렇다면 mutalble 객체 내부에 mutable 객체가 속하는 경우는 어떨까?  
얕은 복사로는 리스트 내부에 리스트가 존재할 때는 값이 동시에 변하는 현상을 막을 수 없다.  

### 2.5.3 깊은 복사

위와 같은 경우 서로 다른 객체가 영향을 주지 않게 복사할 수 있다.  
copy 라이브러리의 `deepcopy()`함수를 사용  
...
copy 라이브러리를 사용하지 않고 깊은 복사 함수를 직접 만들어 보기  
...
두 부분으로 나눠서 생각: 리스트 내 리스트는 윗부분 (행렬로 취급), 리스트가 아닌 자료형은 벡터로 취급  
0으로 행렬로 채우는 `zero_mat()`함수는 아래와 같이 구현  
...
def zero_mat(n, p):
    ...
    return Z
```
NOTE: study_room/linalg_with_python/util.py에 구현  
...
앞에 구현한 함수를 불러오려면, 같은 폴더 내에 파일을 넣고, 라이브러리로 불러 들이면 된다.  
